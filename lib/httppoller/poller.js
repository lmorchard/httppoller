...
var Poller = module.exports.Poller = hp_utils.mkClass({
    ...
    pollOne: function (sub, next) {
        ...
        var fetch_age = now - sub.get('last_fetched');
        if (fetch_age < $this.options.max_fetch_age) {
            // Skip if this was fetched recently.
            return next();
        }
        ...
        var req = mod.request(opts, function (res) {
            ...
            res.on('end', function () {
                ...
                if (302 == res.statusCode || 307 == res.statusCode) {
                    $this.log.debug("REQUEUE TEMPORARY", sub._temporary_url);
